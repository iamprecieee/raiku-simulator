...
const RaikuSimulator = () => {
  ...
  const fetchPlayerStats = useCallback(async () => {
    if (!sessionId) return;
    try {
      const response = await fetchWithCredentials(`${API_BASE}/game/player_stats?session_id=${sessionId}`);
  ...
  const fetchTransactions = useCallback(async (page = 1) => {
    ...
    try {
      ...
      if (showAllTransactions) {
        url = `${API_BASE}/transactions?show_all=true&session_id=${sessionId}&page=${page}&limit=20`;
      } else {
  ...
  const submitJitBid = async () => {
    ...
    const input = prompt(`Enter AOT bid amount (Balance: ${playerStats.balance.toFixed(3)} SOL):`);
    if (!input) return;

    const bidAmount = parseFloat(input);
    if (isNaN(bidAmount) || bidAmount <= 0 || !/^\d+(\.\d+)?$/.test(input.trim())) {
      addNotification('Invalid bid amount! Please enter a valid number.', 'error');
      return;
    }
